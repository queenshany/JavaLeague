...
/**
 * Class AdminCoachUpdateController ~ updates coach in the system window
 * 
 * @author ID: 205791056
 * @author ID: 312181605
 */
public class AdminCoachUpdateController extends AllWindowsController {
	...
	public void initialize() {
		...
		comboCity.getSelectionModel().select(coach.getAddress().getCity());
		comboLevel.getSelectionModel().select(coach.getLevel());

		birthDayDate.setValue(coach.getBirthdate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
		workingDate.setValue(coach.getStartWorkingDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
		String s = "";
		for (String p : coach.getAddress().getPhoneNumber())
			s += p;

		String[] phoneParts = s.split("-");
		phoneText.setText(phoneParts[1]);
		comboPhone.getSelectionModel().select(phoneParts[0]);
		streetText.setText(coach.getAddress().getStreet());
		houseText.setText(Integer.toString(coach.getAddress().getHouseNumber()));
		lastNameText.setText(coach.getLastName());
		firstNameText.setText(coach.getFirstName());
		idText.setText(((Integer) coach.getId()).toString());
	}
	...
	private void submit() {
		...
		String firstName = firstNameText.getText(), lastName = lastNameText.getText(), street = streetText.getText(),
				house = houseText.getText(), phone = phoneText.getText(),
											...
											if (city != null) {
												...
												if (Validation.validName(street)) {
													...
													try {
														...
														if (realHouse > 0) {
															Address address = new Address(city, street, realHouse,
																	realPhone);
															coach.setAddress(address);
															coach.setFirstName(firstName);
															coach.setLastName(lastName);
															coach.setLevel(level);
															coach.setStartWorkingDate(workingD);
															alertLabel.setText("Coach was updated Successfully");

															if (ViewLogic.adminCoachController != null)
																ViewLogic.adminCoachController.setCoachTable();
														} else
