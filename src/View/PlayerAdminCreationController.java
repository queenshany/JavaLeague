 ...
 * @author ID: 312181605
 */
public class PlayerAdminCreationController extends AllWindowsController {
	// ------------------------------ Variables ------------------------------
	@FXML
	...
	private void submit() {
		...
		String idStr = idText.getText(), firstName = firstNameText.getText(), lastName = lastNameText.getText(),
				street = streetText.getText(), house = houseText.getText(), valueStr = valueTextField.getText(),
				phone = phoneText.getText(), areaCode = comboPhone.getSelectionModel().getSelectedItem();
		...
		try {
			...
			if (id > 0) {
				...
				if (!ViewLogic.sysData.getPlayers().containsKey(id)) {
						...
						if (Validation.validName(lastName)) {
							...
							if (birthday != null) {
								...
								if (bd.before(today)) {
															...
															if (value > 0) {
																...
																if (areaCode != null) {
																	...
																	if (Validation.validPhone(phone)) {
																		...
																		if (city != null) {
																			...
																			if (Validation.validName(street)) {
																				...
																				try {
																					...
																					Integer realHouse = Integer
																							.parseInt(house);
																					if (realHouse > 0) {
																						Address address = new Address(
																								city, street, realHouse,
																								realPhone);

																						// adds the player and cleans
																						// the
																						// system
																						if (ViewLogic.sysData.addPlayer(
																							...
																							comboPosition
																									.getSelectionModel()
																									.clearSelection();
																							birthDayDate.getEditor()
																									.clear();
																							workingDate.getEditor()
																									.clear();
																							alertLabel.setText(
																		...
																		alertLabel
																				.setText("Please enter a phone number");
																	else
									...
									alertLabel.setText("Birthdate needs to be before today");
